<template>
  <transition leave-active-class="animate__animated animate__slideOutDown">
    <div ref="tree" class="front-background">
      <!--  登录界面  -->
      <Login style="width: 100%;height: 100%;position: fixed;z-index: 1000;"
             v-show="$store.state.isLogin"/>
      <!--  跟随装饰  -->
      <GIF style="position: absolute;z-index: 999;"
           v-if="this.$store.state.clickRing && this.$store.state.windowOrPhone"/>
      <!--    背景云朵  -->
      ...
      <transition enter-active-class="animate__animated animate__fadeIn">
        <div v-if="this.$store.state.windowSize!=='xs'&&this.wait"
      ...
      >
        <div style="background-color: #e0f2fe;display: flex;justify-content: center">
          <span style="font-size: 22px;font-weight: bold;margin-top: 15px">AHPU 计算机协会</span>
        </div>
        <el-menu :default-active="$route.path"
                 style="min-height: 100%; overflow-x: hidden; font-weight: bold;padding: 20px;"
      ...
      <!--    主页面   -->
      <div ref="sro" class="box-slideUp" style="position: relative;z-index: 2">
        <el-card class="front-main"
