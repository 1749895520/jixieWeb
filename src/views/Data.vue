<template>
  <div type="flex">
    <el-card class="box-card box-opacity">
      <div style="margin-bottom: 10px" :class="{'box-search':this.$store.state.windowOrPhone}">
        <div
          ...
          <el-input suffix-icon="el-icon-search" size="small" placeholder="请输入要查找的协会资料"
      ...
      >
        ...
        <el-table-column sortable prop="name" label="资料标题" min-width="120px">
        </el-table-column>
        <el-table-column prop="content" label="资料内容" min-width="100px">
    ...
    <el-dialog :modal-append-to-body="false"
               title="资料预览" class="box-card" :visible.sync="dialogViewVisible"
...
export default {
  ...
  methods: {
    ...
    save() {
      this.request.post("/official/4", this.form).then(res => {
        if (res.data) {
    ...
    handleEdit(id) {
      let routerGoto = this.$router.resolve('/center/publish/index?type=4&id=' + id)
    ...
    doDelAll() {
      ...
      this.request.post("official/del/batch/4", ids).then(res => {
        if (res.data) {
    ...
    imgAdd(pos, $file) {
      ...
      axios({
        url: 'http://' + serverIp + ':9090/file/upload/data',
        method: 'post',
        data: formData,
    ...
    gotoPublish() {
      let routerGoto = this.$router.resolve('/center/publish/index?type=4')
      window.open(routerGoto.href, '_blank');
    },
  }
}
