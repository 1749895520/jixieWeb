  ...
  <div type="flex">
    <div style="margin-bottom: 10px" :class="{'box-search':this.$store.state.windowOrPhone}">
      <div
        ...
        <el-input suffix-icon="el-icon-search" size="small" placeholder="请输入要查找的协会动态"
    ...
    >
      ...
      <el-table-column sortable prop="name" label="动态标题" min-width="120px">
      </el-table-column>
      <el-table-column prop="content" label="动态内容" min-width="100px">
    ...
    <el-dialog :modal-append-to-body="false"
               title="动态预览" class="box-card" :visible.sync="dialogViewVisible"
...
export default {
  ...
  methods: {
    ...
    save() {
      this.request.post("/official/2", this.form).then(res => {
        if (res.data) {
    ...
    handleEdit(id) {
      let routerGoto = this.$router.resolve('/center/publish/index?type=2&id=' + id)
    ...
    imgAdd(pos, $file) {
      ...
      axios({
        url: 'http://' + serverIp + ':9090/file/upload/news',
        method: 'post',
        data: formData,
    ...
    gotoPublish() {
      let routerGoto = this.$router.resolve('/center/publish/index?type=2')
      window.open(routerGoto.href, '_blank');
    },
  }
}
