  ...
  <div type="flex">
    <div style="margin-bottom: 10px" :class="{'box-search':this.$store.state.windowOrPhone}">
      <div
        ...
        <el-input suffix-icon="el-icon-search" size="small" placeholder="请输入要查找的通知公告"
    ...
    >
      ...
      <el-table-column sortable prop="name" label="通知标题" min-width="120px">
      </el-table-column>
      <el-table-column prop="content" label="通知内容" min-width="100px">
    ...
    <el-dialog :modal-append-to-body="false"
               title="通知预览" class="box-card" :visible.sync="dialogViewVisible"
...
export default {
  ...
  methods: {
    ...
    save() {
      this.request.post("/official/1", this.form).then(res => {
        if (res.data) {
    ...
    handleEdit(id) {
      let routerGoto = this.$router.resolve('/center/publish/index?type=1&id=' + id)
    ...
    doDelAll() {
      ...
      this.request.post("official/del/batch/1", ids).then(res => {
        if (res.data) {
    ...
    imgAdd(pos, $file) {
      ...
      axios({
        url: 'http://' + serverIp + ':9090/file/upload/notice',
        method: 'post',
        data: formData,
    ...
    gotoPublish() {
      let routerGoto = this.$router.resolve('/center/publish/index?type=1')
      window.open(routerGoto.href, '_blank');
    },
  }
}
